    ...
    public void batchDeltaUpsert(List<ProductKeywordStats> statsList) {
        String sql = """
            INSERT INTO product_keyword_stats (
                product_id, keyword, cumulative_orders, cumulative_spend, 
                cumulative_sales, cvr, attribution_type, confidence_score, 
                times_appeared, first_converted_date, last_converted_date
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
            ...
            public void setValues(PreparedStatement ps, int i) throws SQLException {
                ...
                ps.setInt(3, stat.getCumulativeOrders());
                ps.setBigDecimal(4, stat.getCumulativeSpend());
                ps.setBigDecimal(5, stat.getCumulativeSales());
                ps.setBigDecimal(6, stat.getCvr());
                ps.setString(7, stat.getAttributionType());
                ps.setBigDecimal(8, stat.getConfidenceScore());
                ps.setInt(9, stat.getTimesAppeared());
                ps.setDate(10, stat.getFirstConvertedDate() != null ? Date.valueOf(stat.getFirstConvertedDate()) : null);
                ps.setDate(11, stat.getLastConvertedDate() != null ? Date.valueOf(stat.getLastConvertedDate()) : null);
            }
